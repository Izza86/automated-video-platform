...
import {
  ...
  extractVideoMetadataFromBuffer,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!(session && session.user)) {
      ...
      console.warn(
        "No session found for /api/extract-metadata; continuing in dev mode."
      );
    }
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      console.log(
        `Extracting metadata for user: ${session?.user?.id ?? "dev-guest"} | ${(buffer.length / 1024 / 1024).toFixed(1)} MB`
      );
      ...
      try {
        ...
        console.error(
          "JSON body parse failed (likely >10 MB truncation):",
          parseErr
        );
        return NextResponse.json(
          {
      ...
      console.log(
        "Extracting metadata for user:",
        session?.user?.id ?? "dev-guest"
      );
